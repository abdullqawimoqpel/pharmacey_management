<?php
// sample_data_fixed.php - Populate database with sample data (Fixed Version)
session_start();
...
try {
    ...
    echo "<h3>📊 Loading Sample Data (Fixed Version)</h3>";

    // First, let's check what users exist and get their IDs
    $user_check = $pdo->query("SELECT user_id, username FROM users");
    $existing_users = $user_check->fetchAll(PDO::FETCH_ASSOC);

    $success_messages[] = "✅ Found " . count($existing_users) . " existing users";

    // If no users exist besides admin, create additional users first
    ...
    // Get available user IDs for sales
    $available_user_ids = array_column($existing_users, 'user_id');
    ...
    // 5. SAMPLE SALES AND SALE ITEMS (FIXED VERSION)
    $customer_ids = [1, 2, 3, 4, 5, 6, 7, 8];
    $medicine_ids = range(1, 25);
    $payment_methods = ['cash', 'card', 'mobile'];

    $sales_created = 0;

    // Create sales for the past 30 days
    for ($i = 0; $i < 30; $i++) { // Reduced to 30 sales to be safer
        $customer_id = $customer_ids[array_rand($customer_ids)];
        $user_id = $available_user_ids[array_rand($available_user_ids)]; // Use only existing user IDs
        ...
        try {
            ...
            // Skip this sale if there's an error and continue
            continue;
        }
    }
    $success_messages[] = "✅ Generated " . $sales_created . " sample sales with items";
    ...
    // Update customer last purchase dates based on sales
    $update_customer_dates = "UPDATE customers c 
                             JOIN (SELECT customer_id, MAX(sale_date) as last_purchase 
                                   FROM sales GROUP BY customer_id) s 
                             ON c.customer_id = s.customer_id 
                             SET c.last_purchase_date = DATE(s.last_purchase)";
    $pdo->exec($update_customer_dates);
    $success_messages[] = "✅ Updated customer last purchase dates";
} catch (PDOException $e) {
...
echo "<div class='card'>";
echo "<div class='card-header bg-success text-white'><h4>Sample Data Loading Complete (Fixed)</h4></div>";
...
echo "<div class='col-md-6'>";
echo "<ul class='list-unstyled'>";
echo "<li>👤 <strong>" . count($available_user_ids) . " Users</strong> available in system</li>";
echo "<li>💰 <strong>" . $sales_created . " Sample Sales</strong> from the past 30 days</li>";
